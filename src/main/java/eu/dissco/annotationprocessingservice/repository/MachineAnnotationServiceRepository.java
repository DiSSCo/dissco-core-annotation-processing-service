...
public class MachineAnnotationServiceRepository {
  ...
  public Optional<MasInput> getMasInput(String jobId) {
    var query = context.select(MAS_JOB_RECORD_NEW.MAS_ID)
    ...
    return context.select(MACHINE_ANNOTATION_SERVICES.asterisk())
        ...
        .where(MACHINE_ANNOTATION_SERVICES.ID.eq(query))
  ...
  private Optional<MasInput> mapToMasInput(Record masRecord) {
    ...
    try {
      return masInput == null ? Optional.empty()
          : Optional.of(mapper.readValue(masInput.data(), MasInput.class));
    } catch (JsonProcessingException e) {
      throw new DataBaseException(
          "Unable to parse MasInput: " + masRecord.get(MACHINE_ANNOTATION_SERVICES.MAS_INPUT)
              .data());
    }
